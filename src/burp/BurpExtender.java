...
public class BurpExtender implements IBurpExtender, IScannerCheck, ITab, ActionListener, IContextMenuFactory,IMessageEditorController {
    ...
    /*
     * TODO
     * - Update options pane (maybe select payloads type used by active scanner...)
    ...
    public List<IScanIssue> doActiveScan(IHttpRequestResponse baseRequestResponse, IScannerInsertionPoint insertionPoint) {
		...
		if(magicPos > -1 || magicPosBase64 > -1 || magicPosAsciiHex > -1 || magicPosBase64Gzip > -1 || magicPosGzip > -1) {
    		...
    		Set<String> payloadKeys = payloadsSleep.keySet();
    		...
    		while (iter.hasNext()) {
        		...
        		if(magicPos > -1) {
        			newPayload = ArrayUtils.addAll(Arrays.copyOfRange(insertionPointBaseValue, 0, magicPos),payloadsSleep.get(currentKey));
        		} else if(magicPosBase64 > -1) {
        			newPayload = ArrayUtils.addAll(Arrays.copyOfRange(insertionPointBaseValue, 0, magicPosBase64),Base64.encodeBase64URLSafe(payloadsSleep.get(currentKey)));
        		} else if(magicPosAsciiHex > -1) {
        			newPayload = ArrayUtils.addAll(Arrays.copyOfRange(insertionPointBaseValue, 0, magicPosAsciiHex),Hex.encodeHexString(payloadsSleep.get(currentKey)).getBytes());
        		} else if(magicPosBase64Gzip > -1) {
                   newPayload = ArrayUtils.addAll(Arrays.copyOfRange(insertionPointBaseValue, 0, magicPosBase64Gzip),Base64.encodeBase64URLSafe(gzipData(payloadsSleep.get(currentKey))));
                } else {
                    newPayload = ArrayUtils.addAll(Arrays.copyOfRange(insertionPointBaseValue, 0, magicPosGzip),gzipData(payloadsSleep.get(currentKey))); 
                }
        		...
        		if(((int)duration) >= 10){
        			...
        			if(magicPos > -1) {
        				markerStart =  helpers.indexOf(newRequest, helpers.urlEncode(payloadsSleep.get(currentKey)), false, 0, newRequest.length);
        				markerEnd = markerStart + helpers.urlEncode(payloadsSleep.get(currentKey)).length;
        			}else if(magicPosBase64 > -1) {
        				markerStart = helpers.indexOf(newRequest, Base64.encodeBase64URLSafe(payloadsSleep.get(currentKey)), false, 0, newRequest.length);
        				markerEnd = markerStart + helpers.urlEncode(Base64.encodeBase64URLSafe(payloadsSleep.get(currentKey))).length;
        				issueName = issueName + " (encoded in Base64)";
        			} else if(magicPosAsciiHex > -1) {
        				markerStart = helpers.indexOf(newRequest, Hex.encodeHexString(payloadsSleep.get(currentKey)).getBytes(), false, 0, newRequest.length);
        				markerEnd = markerStart + helpers.urlEncode(Hex.encodeHexString(payloadsSleep.get(currentKey)).getBytes()).length;
        				issueName = issueName + " (encoded in Ascii HEX)";
        			} else if(magicPosBase64Gzip > -1) {
                        //Need to use more comprehensive URL encoding as / doesn't get encoded
                        try {
                            markerStart = helpers.indexOf(newRequest, Base64.encodeBase64URLSafe(gzipData(payloadsSleep.get(currentKey))), false, 0, newRequest.length);
                            markerEnd = markerStart + helpers.urlEncode(Base64.encodeBase64URLSafe(gzipData(payloadsSleep.get(currentKey)))).length;
                        ...
                        markerStart = helpers.indexOf(newRequest, gzipData(payloadsSleep.get(currentKey)), false, 0, newRequest.length);
                        markerEnd = markerStart + helpers.urlEncode(gzipData(payloadsSleep.get(currentKey))).length;
            ...
            if((magicPos > -1 && magicPos == bodyOffset) || (magicPosBase64 > -1 && magicPosBase64 == bodyOffset) || (magicPosAsciiHex > -1 && magicPosAsciiHex == bodyOffset) || (magicPosBase64Gzip > -1 && magicPosBase64Gzip == bodyOffset) || (magicPosGzip > -1 && magicPosGzip == bodyOffset)) {
               ...
               while (iter.hasNext()) {
                   ...
                   if(magicPos > -1)    {  
                       ...
                       newBody = ArrayUtils.addAll(Arrays.copyOfRange(request, bodyOffset, magicPos),payloadsSleep.get(currentKey));                
                   } else if(magicPosBase64 > -1) {
                       // Encode the payload in Base64
                       newBody = ArrayUtils.addAll(Arrays.copyOfRange(request, bodyOffset, magicPosBase64),Base64.encodeBase64URLSafe(payloadsSleep.get(currentKey)));
                   } else if(magicPosAsciiHex > -1) {
                       // Encode the payload in Ascii HEX
                       newBody = ArrayUtils.addAll(Arrays.copyOfRange(request, bodyOffset, magicPosAsciiHex),Hex.encodeHexString(payloadsSleep.get(currentKey)).getBytes());
                   } else if(magicPosBase64Gzip > -1) {
                        // Encode/compress the payload in Gzip and Base64
                        newBody = ArrayUtils.addAll(Arrays.copyOfRange(request, bodyOffset, magicPosBase64Gzip),Base64.encodeBase64URLSafe(gzipData(payloadsSleep.get(currentKey))));
					} else {
						// Encode/compress the payload with Gzip
					    newBody = ArrayUtils.addAll(Arrays.copyOfRange(request, bodyOffset, magicPosGzip),gzipData(payloadsSleep.get(currentKey)));
					}
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(command.equals("attack")) {
			...
			if(choice == 0 ){
				...
				Thread t = new Thread() {
				    public void run() {
				    	executeManualTest(BurpExtender.TYPE_BASE64, (String)(typeManualTest.getSelectedItem()));
				    }
				};
				t.start();
			...
			if(choice == 0 ){
				...
				Thread t = new Thread() {
				    public void run() {
				    	executeManualTest(BurpExtender.TYPE_ASCII_HEX, (String)(typeManualTest.getSelectedItem()));
				    }
				};
				t.start();
			...
			if(choice == 0 ){
	            ...
	            Thread t = new Thread() {
	                public void run() {
	                    executeManualTest(BurpExtender.TYPE_BASE64GZIP, (String)(typeManualTest.getSelectedItem()));
	                }
	            };
	            t.start();
			...
			if(choice == 0 ){
	            ...
	            Thread t = new Thread() {
	                public void run() {
	                    executeManualTest(BurpExtender.TYPE_GZIP, (String)(typeManualTest.getSelectedItem()));
	                }
	            };
	            t.start();
	...
	public void executeManualTest(int encoding, String testType) {		
		...
		if(payloadFrom != payloadTo) {
			...
			if(testType.equals(BurpExtender.TEST_SLEEP)) {
	    		...
	    		while (iter.hasNext()) {
	    			...
	    			dnsPayloadWithCollaboratorPayload = createDnsVector(currentValue,currentCollaboratorPayload);
    		...
    		while (iter.hasNext()) {
    			...
    			if(encoding == BurpExtender.TYPE_RAW) {
    				request = ArrayUtils.addAll(prePayloadRequest,currentPayloads.get(currentKey));
    			} else if(encoding == BurpExtender.TYPE_BASE64) {
    				request = ArrayUtils.addAll(prePayloadRequest,Base64.encodeBase64URLSafe(currentPayloads.get(currentKey)));
    			} else if(encoding == BurpExtender.TYPE_ASCII_HEX) {
    				request = ArrayUtils.addAll(prePayloadRequest,Hex.encodeHexString(currentPayloads.get(currentKey)).getBytes());
    			} else if(encoding == BurpExtender.TYPE_BASE64GZIP) {
                    request = ArrayUtils.addAll(prePayloadRequest,Base64.encodeBase64URLSafe(gzipData(currentPayloads.get(currentKey))));
                } else {
                    request = ArrayUtils.addAll(prePayloadRequest,gzipData(currentPayloads.get(currentKey)));
                }
        		...
        		*/
        		...
        		if( ( ( testType.equals(BurpExtender.TEST_SLEEP) || testType.equals(BurpExtender.TEST_CPU) ) && (((int)duration) >= 10) ) || ( testType.equals(BurpExtender.TEST_DNS) && ( collaboratorInteractions.size() > 0 ) ) ) {
        			...
        			if(addManualIssueToScannerResultManualTesting.isSelected()) {
        				...
        				String issueName = activeScanIssue;
        				if( testType.equals(BurpExtender.TEST_SLEEP) || testType.equals(BurpExtender.TEST_DNS)) {
        					issueName = issueName + activeScanIssueVulnerableLibrary + currentKey;       					
        				}
        				
        				if(encoding == BurpExtender.TYPE_BASE64) {
        					issueName = issueName + " (encoded in Base64)";
        				} else if(encoding == BurpExtender.TYPE_ASCII_HEX) {
        					issueName = issueName + " (encoded in Ascii HEX)";
        				} else if(encoding == BurpExtender.TYPE_BASE64GZIP) {
                            issueName = issueName + " (encoded in Base64 and Gzipped)";
                        } else if(encoding == BurpExtender.TYPE_GZIP) {
                            issueName = issueName + " (encoded/compressed with Gzip)";
                        }
	...
	public byte[] createDnsVector(byte[] origVector, String collaboratorURL) {
